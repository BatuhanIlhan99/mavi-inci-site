(function () {
  ...
  var featureLibrary = [
    ...
    { icon: '02', title: 'Butik Servis', text: 'Kucuk olcegin avantaji ile daha hizli geri donus ve kisilestirilmis yonlendirme.' },
    { icon: '03', title: 'Yavas Kahvalti', text: 'Yerel urunler ve uzun oturuslara uygun sakin sabah akisi.' },
    { icon: '04', title: 'Gun Batimi Terasi', text: 'Aksam saatlerinde otelin tonunu belirleyen en guclu ortak alan deneyimi.' }
  ];

  function renderPageHero() {
    return '<section class="page-hero page-hero-experiences"><div class="container page-hero-grid"><div class="page-hero-copy"><div class="hero-prelude"><span class="hero-chip">Deneyimler ve Teklifler</span><span class="hero-rule"></span><span class="hero-annotation">Iyi otel sitelerinde odalar kadar deneyim, teklif ve mekan ritmi de net okunur</span></div><p class="kicker">Deneyimler sayfasi</p><h1 class="hero-title room-page-title">Konaklamayi tamamlayan deneyimler ve sezonsal avantajlar</h1><p class="hero-text">Bu sayfada otelin servis basliklarini, galeri akisini, imza konaklama paketlerini ve fiyat mantigini bir araya topladik. Amac yalnizca guzel gormek degil; misafirin neyi neden sectigini netlestirmek.</p><div class="hero-actions"><a class="button button-primary" href="' + Shell.pageHref('booking') + '">Rezervasyon Sayfasina Gec</a><a class="button button-secondary" href="#galeri">Galeriyi Incele</a></div></div><aside class="page-hero-aside"><article class="contact-card"><p class="section-kicker">Bugunun Ruhu</p><h3>Deneyim basliklari</h3><div class="summary-line"><span>Sahil</span><strong>' + (state.hotel.beachService ? 'Servis destekli sakin akisi' : 'Sakin sahil ritmi') + '</strong></div><div class="summary-line"><span>Kahvalti</span><strong>' + Data.escapeHtml(state.hotel.breakfastHours) + '</strong></div><div class="summary-line"><span>Transfer</span><strong>' + (state.hotel.airportTransfer ? 'Not alinabilir' : 'Standart ulasim') + '</strong></div><div class="summary-line"><span>Direkt Hattiniz</span><strong>' + Data.escapeHtml(state.hotel.phone) + '</strong></div></article></aside></div></section>';
  }

  function renderFeatures() {
    return '<section class="section section-dark"><div class="container"><div class="section-header"><p class="section-kicker">Otel Deneyimi</p><h2 class="section-title">Sadece bir oda degil, tam bir konaklama ritmi</h2><p class="section-text">Dunyanin guclu otel sitelerinde deneyim katmani yalnizca ek bilgi degil, karar anini yoneten bir bilesen gibidir. Bu alani da buna gore yeniden kurguladik.</p></div><div class="features-grid">' + featureLibrary.map(function (item) { return '<article class="feature-card"><div class="feature-icon">' + Data.escapeHtml(item.icon) + '</div><h3>' + Data.escapeHtml(item.title) + '</h3><p>' + Data.escapeHtml(item.text) + '</p></article>'; }).join('') + '</div></div></section>';
  }
  ...
  function renderGallery() {
    ...
    return '<section class="section section-light" id="galeri"><div class="container"><div class="gallery-heading"><div><p class="section-kicker">Galeri</p><h2>Otelin tonunu anlatan secili kareler</h2></div><p class="section-text">Galeri sayfasi yerine deneyimler sayfasina entegre bir kurgu yaptik; boylece oda, ortak alan ve sahil atmosferi ayni akista gezilebilir.</p></div><div class="gallery-filters" id="galleryFilters">' + categories.map(function (category, index) { return '<button class="filter-chip' + (index === 0 ? ' is-active' : '') + '" type="button" data-gallery-filter="' + Data.escapeHtml(category) + '">' + Data.escapeHtml(category) + '</button>'; }).join('') + '</div><div class="gallery-grid" id="galleryGrid">' + galleryItems.map(renderGalleryCard).join('') + '</div></div></section>';
  }
  ...
  function renderLocalGuide() {
    ...
    return '<section class="section section-light section-destination"><div class="container"><div class="section-header"><p class="section-kicker">Mekanin Ruhu</p><h2 class="section-title">Deneyim sayfasini yalnizca imkan listesi olmaktan cikardik</h2><p class="section-text">Luks segmentte iyi otel siteleri yalnizca ne sundugunu degil, ne hissettirdigini de aktarir. Bu nedenle deneyim sayfasina daha editorial bir akis ekledik.</p></div><div class="destination-grid">' + cards.map(function (item) { return '<article class="destination-card"><h3>' + Data.escapeHtml(item.title) + '</h3><p>' + Data.escapeHtml(item.text) + '</p><span>' + Data.escapeHtml(item.meta) + '</span></article>'; }).join('') + '</div></div></section>';
  }
  ...
  function renderApp() {
    return '<div class="shell">' +
      ...
      Shell.renderFloatingActions(state, { primaryHref: Shell.pageHref('booking'), primaryTitle: 'Rezervasyon', primaryLabel: 'Teklifle Devam Et', secondaryLabel: 'Deneyim Icin Yaz' }) +
