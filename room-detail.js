(function () {
  ...
  function bookingHref() {
    return './rezervasyon.html?room=' + encodeURIComponent(Data.getRoomKey(room));
  }
  ...
  function buildRoomStructuredData() {
    ...
    var sizeMatch = String(room.size || '').match(/(\d+(?:[.,]\d+)?)/);
    var sizeValue = sizeMatch ? parseFloat(sizeMatch[1].replace(',', '.')) : NaN;
    return [
      {
        ...
        itemListElement: [
          ...
          { '@type': 'ListItem', position: 2, name: 'Odalar', item: Data.getSiteUrl(state, '/odalar.html') },
